...
<div class="hasflex" id='main'>
	...
	if(isset($_POST['submit'])){
		...
		if(!isset($error)){
			...
			try {
				...
				$stmt = $db->prepare('INSERT INTO blog_posts_seo (postThumb,postImage,postTitle,postSlug,postDesc,postCont,postDate) VALUES (:postThumb,:postImage, :postTitle, :postSlug, :postDesc, :postCont, :postDate)') ;
				$stmt->execute(array(
    ...
    <div id="urlbuttons" class="imageSelect">
    <button class="myBtn" onclick="openModal(['urlImage','urlUpload'])">Generate URL for image</button>
    </div>
    <div id="urlBox" class="hasflex imageSelect">
    <input type="text" name="urlImage" placeholder="Paste URL into add image on the editor" readonly>
    </div>
    
    <div id="buttons" class="imageSelect">
	<p>Select image to upload from website:</p>
	<button class="myBtn" onclick="openModal(['mainImage','mainUpload'])">Choose Main Image</button>
    <!--<button class="myBtn" onclick="openModal(['thumbImage','thumbUpload'])">Choose Thumbnail Image</button>-->
